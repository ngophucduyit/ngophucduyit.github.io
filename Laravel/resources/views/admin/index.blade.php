		...
		<div class="col-sm-9 bg-white border rounded">
			...
			<div id="chart">
				<script type="text/javascript">
    				...
    				function open_py(){
						swal({
							  title: "Chuyển đến File Update?",
							  text: "Thao tác trên File có thể ảnh hưởng đến Dữ Liệu. Tiếp tục?",
							  icon: "warning",
							  buttons: true,
							})
							.then((willDelete) => {
							  if (willDelete) {
							  	window.open("http://localhost:8888/notebooks/crawl_vuighe.ipynb");
							  } 
							  else {
							    swal("Đã hủy!!!");
							  }
    					...
    					for (var i = 0; i < view.length; i++) {
    						for (var j = i + 1; j < view.length; j++) {
    							if (view[i] < view[j]){
    								...
    								[link_am[i],link_am[j]] = [link_am[j],link_am[i]]
    							}
    						}
    					}
    					for (var i = 0; i < name.length; i++) {
    						$('marquee#scroll').append('<div class = "border mb-3 rounded"><p id="name" class=" pt-3 text-center"><b><a href ="'+link_am[i]+'" class="mlem" target="_blank">'+name[i]+'</a></b></p><hr><div class="row" style="font-size: 13px"><div class="col-sm-6 border-right"><img src = "'+img[i]+'" width="100%"><br>Thể loại: '+ type[i].join(', ') +'</div><div class="col-sm-6"><p>View: '+Intl.NumberFormat().format(view[i])+'</p><p>Like: '+Intl.NumberFormat().format(like[i])+'</p><p>Follow: '+Intl.NumberFormat().format(follow[i])+'</p><p>Thời Lượng: '+num_ep[i]+' tập</p></div></div></div>')
    					}
					})
					</script>
				</marquee>
						...
						<script type="text/javascript">
							function getinput(){
								...
								$.get('admin/getdefault/'+ val, function(data)
								{
									...
									$('div#default').append('<div class="col-sm-8 text-left" id="text"><h4><a class="mlem" target ="_blank" href = "'+link_am+'">'+ name +'</a></h4><p>'+ des +'</p><p>Lượt xem: '+ Intl.NumberFormat().format(view_am) +'</p><p>Like: '+ Intl.NumberFormat().format(like) +'</p><p>Follow: '+ Intl.NumberFormat().format(follow) +'</p><p>Thời Lượng: '+ num_ep +' tập</p></div>');
			    				});
							}
						</script>
					</form>
